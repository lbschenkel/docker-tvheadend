...
RUN \
  ...
  if [ -z ${TVHEADEND_COMMIT+x} ]; then \
    TVHEADEND_COMMIT=$(curl -sX GET https://api.github.com/repos/lbschenkel/tvheadend/commits/master \
    | jq -r '. | .sha'); \
  fi && \
  mkdir -p \
    /tmp/tvheadend && \
  git clone https://github.com/lbschenkel/tvheadend.git /tmp/tvheadend && \
  cd /tmp/tvheadend && \
